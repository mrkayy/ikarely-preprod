...
const Covid19 = ({ show, user, uid }) => {
  ...
  useEffect(() => {
    ...
    setSchemas((prevState) => ({
      ...
      countryOfTravel: Joi.string().required().label("countryOfTravel"),
      address: Joi.string().required().label("address"),
      nearestBusstop: Joi.string().required().label("nearestBusstop"),
      dateOfTravel: Joi.string().required().label("dateOfTravel"),
  ...
  const sendRequest = () => {
    const requestData = {
      ...
      request_form: {
        ...
        screening_purpose: data.purpose,
        nationality: data.nationality,
        country_of_travel: data.countryOfTravel,
          ...
          <>
            <form onSubmit={nextStage}>
              <InputBox
                ...
                hasOption={false}
                name="address"
                placeholder="please enter your preferred location for the screening"
                labelColor={"text-typography-bold"}
              />
              <InputAreaBox
                type="text"
                label="What is the Purpose for the screening?"
                rows={4}
                name="purpose"
                placeholder=""
                required={true}
        ...
        return (
          <>
            <div className="h-5/6 min-w-full grid grid-cols-1 place-content-between">
              <div className="flex-col justify-evenly mt-6">
              ...
              {loading ? (
                <div className="text-typography-emphasis text-xl text-center w-full">
                  Requesing for Covid19 Screening ...
                </div>
              ) : (
                <>
                  {" "}
  ...
  return (
    <CustomServiceModal showModal={show} title="Request Covid-19 Screening">
      {stagesRender(stage)}
    </CustomServiceModal>
  );
};

export default observer(Covid19);
